...
int main() {
    ...
    while (true) {
        ...
        while (delimiterPos != string::npos) {
            string recv_recipient = received_message.substr(0, delimiterPos);
            string message = received_message.substr(delimiterPos + 1);

                   cout << "Message from " << recv_recipient << ": " << message << endl;
        ...
        if (send(client_socket, combined_data.c_str(), combined_data.length(), 0) == SOCKET_ERROR) {
